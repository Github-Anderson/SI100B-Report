...
def image_split_column(img:np.ndarray)->list:
    ...
    #+15/-15 to create a frame for the image, improving the recognition rate.
...
def led_display(numList:list)->None:
    ...
    for segment in seg:
        ...
        GPIO.output(segment, True) # initialize the GPIOs representing the leds
    num0 = [a,b,c,d,e,f] 
    num1 = [b,c]
    num2 = [a,b,g,e,d]
    num3 = [a,b,g,c,d]
    ...
    # The list of led pins which should light up to display each number
    ...
    for ele in numList: # for each number in the number list
        for dig in n_list[ele]:
            GPIO.output(dig, GPIO.LOW) # turn on the led
        #The initial voltage is high, so setting the output to GPIO.LOW creates a circuit and lights up the led.
        time.sleep(1) # wait for 1 second
        for dig in n_list[ele]:
            GPIO.output(dig, GPIO.HIGH) # turn off the led
        time.sleep(1) # wait for 1 second
    
    GPIO.cleanup() # clean up the GPIOs used in led display
    ret = None
    return ret
...
def take_photo()->str:
    ...
    GPIO.setup(40,GPIO.OUT) #initialize the GPIOs representing the button and indicator led light
    ...
    # clear the camera
    with PiCamera() as camera: # create camera
             
        camera.start_preview() # preview
        while True:  # wait for the button to take the photo
            
            if GPIO.input(7) == 1: # if the button is pressed
                GPIO.output(40,GPIO.HIGH) # turn on the indicator led light
                sleep(1)
                GPIO.output(40,GPIO.LOW)  # turn off the indicator led light
               
                
                sleep(1) # wait for 1 second
                camera.stop_preview() # stop previewing
                filename = 'photo.jpg' # create image file
                camera.capture(PRJ_PATH + filename) # take the photo and save it
                   
                GPIO.cleanup() # clean up the GPIOs used in taking the photo
